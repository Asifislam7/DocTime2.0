...
class ChatbotService {
     ...
     */
    static async generateAIResponse(userInput, userData, appointments) {
        try {
...
- Platform navigation and features`
     ...
     */
    static createPrompt(userInput, userData, appointments) {
        const context = `
...
APPOINTMENTS:
${appointments.length > 0 ? appointments.map(apt => `- ${apt.doctorName} on ${new Date(apt.appointmentDate).toLocaleDateString()} at ${apt.appointmentTime} (${apt.status}) - ${apt.reason}`).join('\n') : 'No appointments found'}
...
Please provide a helpful response following the rules above.`;
     ...
     */
    static generateFallbackResponse(userInput) {
     ...
     */
    static async summarizeDocument(documentText) {
        try {
            const openai = this.initializeOpenAI();
            const completion = await openai.chat.completions.create({
                model: 'gpt-3.5-turbo',
